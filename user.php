...
<html>
<head>
<title>Изменение данных <?php echo $email; ?></title>
</head>
<body>
<?php
...
////////Изменение пароля
...
else if (isset($_POST['password'])){//Если существует имя
	...
	if ($mdPassword == '') {
		exit("Вы не ввели пароль<br><a href='edit.php'>Вернуться назад</a>");
	}

	$up = mysql_query("UPDATE users SET password='$mdPassword' WHERE id='$id_user'");//обновляем имя
	if ($up == true) {
		echo "<meta http-equiv='Refresh' content='0; URL=profile.php?id=".$id_user."'>";
	}
}
